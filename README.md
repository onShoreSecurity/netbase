...
Netbase can attach static labels to monitored hosts so observations carry role, site
or OS context. Labels show up in the `ip_labels` field and make peer-group comparison
possible ("what is normal for a domain controller?").

```
redef Netbase::cidr_labels += {
    [10.1.0.0/16]   = set("site:hq", "role:workstation"),
    [10.9.9.0/24]   = set("role:dc", "os:windows"),
};
```

Labels can also be maintained in a TSV file that is re-read live, so they can be
updated without restarting Zeek:

```
redef Netbase::cidr_labels_file = "/opt/zeek/etc/netbase-labels.tsv";
```

# Detection and Tuning

Netbase can close the loop and alert on activity a host has never exhibited before.
This is **opt-in** and requires a Zeek built with the Redis storage backend, because
the baseline is shared across processes (see below).

```
@load netbase/detect
```

Out of the box this runs in **learning mode**: it populates the baseline and raises
nothing. Once the store has seen a representative stretch of normal activity, turn
reporting on:
...
You then get Notices the first time a monitored host contacts a new external ASN or
country, uses a new destination port, or runs new software.

## Tuning

Tuning is done by suppressing known-good values, not by turning dimensions off. Add
entries globally or per host:

```
redef Netbase::suppressions += {
    "ext_asn=15169",                  # Google, everywhere
    "10.1.2.3|ext_country=IE",        # this host legitimately talks to Ireland
};
```

Analysts can maintain these in a file that is re-read live, making it the tuning
artifact for the workflow:

```
redef Netbase::suppressions_file = "/opt/zeek/etc/netbase-suppressions.tsv";
```

Additional safety valves: `Netbase::learning_window` (warm-up after start, so a
restart never alerts on the world), `Netbase::baseline_ttl` (how long a learned value
stays "known"), and `Netbase::max_checks_per_dimension` (bounds the work a single
noisy host can trigger).

## A note on parallel workers

Netbase is often run as several independent Zeek workers over AF_PACKET fanout with
no cluster. In that topology each worker sees only a slice of a given host's traffic,
so per-host observations are split across workers and **merged downstream**.

The detection state deliberately lives in Redis rather than worker memory, and
first-seen checks use an atomic test-and-set, so parallel workers share one consistent
baseline and a new value is reported at most once. Configure the connection with
`Netbase::redis_server_host` / `redis_server_port` (defaults to the sensor's local
instance).

Note that *numeric* thresholds should be evaluated downstream where per-worker rows
have been merged — a single worker's counts are only a fraction of the host's true
totals. See `ROADMAP.md` for details.
